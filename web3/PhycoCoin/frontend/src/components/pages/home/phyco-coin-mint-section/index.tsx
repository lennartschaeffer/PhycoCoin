...
export default function PhycoCoinMintSection() {
    ...
    const { walletConnectionStatus, switchNetwork, chainCurrent, walletAddress } = useWallet();
    ...
    const onMint = async () => {
        ...
        try {
            ...
            if (walletConnectionStatus === "connected" && switchNetwork && deployedNetworkData) {
                ...
                if (receipt) {
                    ...
                    console.log(`New balance of ${walletAddress}: ${newBalance.toString()} PHYC`);
                    console.log("Minting transaction successful! Transaction Hash:", receipt.hash);

                    setMintAmount("");
                    alert(`Successfully minted ${mintAmount} PHYC to ${walletAddress} (Tx: ${receipt.hash.slice(0, 6)}...${receipt.hash.slice(-4)})`);
                } else {
            ...
            <div className={styles.mintSectionInner}>
                ...
                <form className={styles.mintForm} onSubmit={(e) => {
                    ...
                    <input
                        ...
                        disabled={isMinting || walletConnectionStatus !== "connected"}
                    />
                    <button
                        className={styles.btn}
                        disabled={!mintAmount || isMinting || walletConnectionStatus !== "connected"}
                        type="submit"
                    >
                        Mint
                    </button>
